...
describe "rcs03a program structure" do
  ...
  it "reports back traces containing each instruction in the path" do
    #puts call_stack_info[4].worst_case_backtraces.join("\n")
    addrs = call_stack_info[4].worst_case_backtraces.collect(&:addresses)
    addrs.should == [[4, 0x100], [4, 0x100, 0x101], [4, 0x100, 0x101, 0x102]]

    addrs = call_stack_info[0].worst_case_backtraces.collect(&:addresses)
    addrs.should == [
  ...
  it "can report interesting backtraces" do
    addrs = call_stack_info[0].interesting_backtraces.collect(&:interesting_addresses)
    addrs.should == [
      [0, 36, 64, 65, 96, 128, 256]
    ]
  end
  
  it "can report interesting backtraces as a string with #worst_case_examples_report" do
    call_stack_info[0].worst_case_examples_report.should == <<END
Backtrace:
