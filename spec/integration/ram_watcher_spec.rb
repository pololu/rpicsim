...
# This is an integration test for Pic#ram_watcher,
# so it tests the Pic class, MemoryWatcher class, and MPLAB X.
...
if !RPicSim::Flaws[:fr_memory_attach_useless]
  ...
  describe "Pic#ram_watcher" do
    ...
    it "sees when the firmware writes to RAM variables" do
      ...
      y.value = 20
      step
      pic.ram_watcher.clear
      run_subroutine :addition, cycle_limit: 100
      pic.ram_watcher.writes.should == { z: 275 }
    end
    ...
    it "sees when the firmware writes to an unnamed part of RAM" do
      ...
      pic.ram_watcher.writes.should == { 0x5F => 44 }
    end
    ...
    it "sees when the firmware writes to an SFR" do
      ...
      pic.ram_watcher.writes[:TRISA].should == 14
    end
    ...
    it "confuses writes to LATA with writes to PORTA when ANSELA bits are 1 and TRISA bits are 1", flaw: true do
      ...
      pic.ram_watcher.writes[:PORTA].should == 0
      pic.ram_watcher.writes[:LATA].should == nil
    end
  end
