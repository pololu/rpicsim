...
describe 'RPicSim::Sim#ram' do
  ...
  it 'can write to RAM and then be read by the firmware' do
    ram.write_byte program_file.var_addresses[:xu16], 70
    ram.write_byte program_file.var_addresses[:xu16] + 1, 0
    ram.write_byte program_file.var_addresses[:yu16], 22
    ram.write_byte program_file.var_addresses[:yu16] + 1, 0
    run_subroutine :addition, cycle_limit: 100
    expect(ram.read_byte(program_file.var_addresses[:zu16])).to eq 92
    expect(ram.read_byte(program_file.var_addresses[:zu16] + 1)).to eq 0
  end
