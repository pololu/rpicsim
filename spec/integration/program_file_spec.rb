...
describe 'RPicSim disassembly' do
  let(:address) { @program_file.label('ins_' + instruction_name).address }
  ...
  context 'for PIC18 architecture' do
    ...
    describe 'goto' do
      ...
      it 'has the opcode GOTO' do
        expect(instruction0.opcode).to eq 'GOTO'
      end
      ...
      it 'has a size of 4 because it takes 2 words' do
        # the PIC18 stores flash addresses in terms of bytes, not words
        expect(instruction0.size).to eq 4
      end
      ...
      it 'includes the Goto module' do
        expect(instruction0).to be_a_kind_of RPicSim::Instruction::Goto
      end
    end
    
    conditional_skips_fa.each do |name|
      specify name do
        address = @program_file.label('ins_' + name).address
        i0 = @program_file.instruction(address)
        expect(i0.address).to eq address
        expect(i0.opcode).to eq name.upcase
        ...
        expect(i1.opcode).to eq name.upcase
