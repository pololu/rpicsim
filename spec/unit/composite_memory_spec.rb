...
describe RPicSim::CompositeMemory do
  ...
  it 'uses the first memory if it can, without considering the second' do
    expect(@mem1).to receive(:is_valid_address?).with(0x40).and_return(true).exactly(2).times
  ...
  it 'uses the second memory if needed' do
    expect(@mem1).to receive(:is_valid_address?).with(0x40).and_return(false).exactly(2).times
    expect(@mem2).to receive(:is_valid_address?).with(0x40).and_return(true).exactly(2).times
  ...
  describe 'is_valid_address?' do
    it 'returns false if the address is invalid in all component memories' do
      expect(@mem1).to receive(:is_valid_address?).with(0x40).and_return(false)
      expect(@mem2).to receive(:is_valid_address?).with(0x40).and_return(false)
      expect(@composite_memory.is_valid_address?(0x40)).to eq false
    end
    
    it 'returns true if the address is valid in either memory' do
      expect(@mem1).to receive(:is_valid_address?).with(0x40).and_return(false)
      expect(@mem2).to receive(:is_valid_address?).with(0x40).and_return(true)
      expect(@composite_memory.is_valid_address?(0x40)).to eq true
    end
  end
